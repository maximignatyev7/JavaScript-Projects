...
//square has already been assigned and if has not, record new square with the assigned avatar.
...
function checkForWinCon() {
    ...
    info.sort(); // sort the square array in order despite the actual gameplay sequance
    for (var i in info) {
...
//whenever a win is detected the correcponding function will
...
function winner(winDetected, winCon) {
    if (winDetected == "win") {
        ...
        glowBoard(winCon); // call function to make the gameboard pulse with color
    }
}
...
// these finctions will produce game sounds depending on the occasion
...
// -- These function are the algoritms to find all win conditions --//
...
function checkWinCon1(info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "0") {
            var match0Avatar = info[i].charAt(1); // only intrested in recording the avatar
        }
...
function checkWinCon3(info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "6") {
            var match6Avatar = info[i].charAt(1); // only interested in the avatar
        }
...
// checking for wincon aquares 048
...
// checking for wincon aquares 642
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { //if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty than the square is unoccupied.
            ...
            if (paintAvatar == "O") { // change these all to ternary statements insted
            ...
            checkForWinCon(); //call function tocheck if current move completes a winning condition.
