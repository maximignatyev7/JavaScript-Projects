function getReceipt() {
    ...
    // these variables will be passed on to each fuction
...
function getVegetable(runningTotal,text1) {
    ...
    var vegetableArray = document.getElementsByClassName("vegi");
    for (var f = 0; f<vegetableArray.length; f++) {
        if (vegetableArray[f].checked) {
            selectedVegetable.push(vegetableArray[f].value);
            console.log("selected vegetable item: ("+vegetableArray[f].value+")");
            text1 = text1+vegetableArray[f].value+"<br>"
        }
    }
